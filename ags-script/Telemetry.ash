...
import String Telemetry_ServerUrl;            // Remote telemetry server (empty = local file only); needs TELEMETRY_REMOTE + AGS Telemetry plugin
import String Telemetry_ApiKey;               // Bearer key for the telemetry server (empty if none)
...
import void Telemetry_LogRoomInteraction(int x, int y, CursorMode mode, bool usedDefaultCatchAll);  // hotspots, objects and characters
